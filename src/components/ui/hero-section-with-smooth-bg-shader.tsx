...
export function HeroSection({
  ...
  onButtonClick,
  // Adjusted default colors to match a cool, modern palette, but you can override these
  ...
  fontFamily = "inherit", // Changed to inherit to use your project's fonts
  fontWeight = 500,
}: HeroSectionProps) {
    ...
    <section className={`relative w-full min-h-screen overflow-hidden bg-background flex items-center justify-center ${className}`}>
      ...
      <div className={`relative z-10 ${maxWidth} mx-auto px-6 w-full`}>
        <div className="text-center">
          ...
          >
            {title} <span className="text-[#FF6B00]">{highlightText}</span>
          </h1>
          <p className={`text-lg sm:text-xl text-slate-800 dark:text-slate-200 text-pretty max-w-2xl mx-auto leading-relaxed mb-10 px-4 ${descriptionClassName}`}>
            {description}
          </p>
          <button
            onClick={handleButtonClick}
            className={`px-6 py-4 sm:px-8 sm:py-6 rounded-sm bg-[#FF6B00] text-sm sm:text-base text-white hover:bg-orange-700 transition-colors shadow-xl ${buttonClassName}`}
          >
            {buttonText}
          </button>
        </div>
